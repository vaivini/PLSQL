...
WITH PARAMETROS AS
 ...
 PCEST2 AS (
    SELECT PCEST.DTULTFAT, PCEST.QTESTGER, PCEST.QTRESERV, PCEST.QTINDENIZ, PCEST.QTBLOQUEADA,PCEST.QTPENDENTE, PCEST.CUSTOULTENT,
...
SELECT DISTINCT
     ...
     --  SUM(((NVL(PCEST2.QTESTGER,0) - NVL(PCEST2.QTRESERV,0) -  (CASE WHEN PARAMETROS.BLOQUEIAVENDAESTPENDENTE = 'S' THEN  NVL(PCEST2.QTPENDENTE, 0)  ELSE 0 END) ))) QTESTOQUE,
  (SUM(PCEST2.QTESTGER)  -   SUM(PCEST2.QTBLOQUEADA) ) AS ESTOQUE_REAL  ,
    
         SUM(PCEST2.QTINDENIZ) INDENIZADA,
 ...
 AND PCPRODUT.DTEXCLUSAO IS NULL
   ...
   AND 'N' IN NVL(       (         SELECT           PF.FORALINHA         FROM            PCPRODFILIAL PF          WHERE            PF.CODPROD = PCPRODUT.CODPROD AND           PF.CODFILIAL = PCEST2.CODFILIAL       )     , 'N')
